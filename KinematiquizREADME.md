...
2. When encountering a graphing question, it will ask if you want a solution. If you say "yes", it will ask for one value and when entering a value it will input it in a y function and show the y solution
 and the program asks the user for another value and the same program ouput is given. Then the user is challenged to figure out the equation based 
3.  An example for a graphing question is a user enters in 2 and the hidden y function is 2x, then the solution would show 4. It is highly recommended to first ask the value 0 to see if there's any constant(e.g. y = 2x+3) and
   then based on the pattern you see in the graph input a guess such as 2. If you get 4, subtract that from the first solution and divide that by the difference of the first and
  second values((4-0)/(2-0)= 2) to calculate the slope. Since the solution at 0 is 0 the constant is 0. Therefore the equation is y = 2x.It is then up to the student or user who is expected to know physics
  concepts to use that equation and plug in values figured out in the question. This question is one of the most intensive questions and requires the student or user to have studied kinematics very well and use
  this program as a way to test their knowledge to prepare for the quiz. Calculus is very helpful, but at least geometry
  and algebra would be required to solve these type of problems.
4. The program has 10 questions but runs 21 times and some of the questions can repeated. This is more of a helpful tool for students to quiz themselves.
